...
CallbackReturn KukaRSIHardwareInterface::on_activate(
  ...
  RCLCPP_WARN(rclcpp::get_logger("KukaRSIHardwareInterface"), "I am PID: %lu",
              pthread_self());
  cpu_set_t cpu_set;
  CPU_SET(9, &cpu_set);
  CPU_SET(10, &cpu_set);
...
return_type KukaRSIHardwareInterface::write(const rclcpp::Time &,
  ...
  for (size_t i = 0; i < info_.joints.size(); i++) {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("joints"), "%li:\t%f\t(%f, %f)", i,
    //             joint_pos_correction_deg_[i], hw_commands_[i],
    //             initial_joint_pos_[i]);
    // RCLCPP_INFO(rclcpp::get_logger("joints"), "%li:\t%f", i,
    //             joint_pos_correction_deg_[i] - prev_corrs_vals_[i]);
  }
  // RCLCPP_INFO(rclcpp::get_logger("joints"), "done");
